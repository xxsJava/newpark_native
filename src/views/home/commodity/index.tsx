...
import { SafeAreaView,View,Text,StyleSheet,Dimensions,ScrollView,Image,Platform } from 'react-native';
    ...
    return (
        <View style={styles.safeAreaStyle}>
            <Appbar.Header style={styles.headerStyle}>
                <Appbar.Action icon={require('../../../assets/images/chevron-left.png')} onPress={() => navigate('HomeStacker')} />
                ...
                {typeData.map(item => {
                    return (
                        <View style={styles.typeItem} key={item.index}>
                            <Text style={[styles.typeText,styles.typeText1]}>{item.text}</Text>
                            <View>
                                <Icon size={10} color='#888' source={require('../../../assets/images/triangle-up.png')} />
                                <Icon size={10} color='#888' source={require('../../../assets/images/triangle-down.png')} />
                            </View>
                            {/* <Entypo size={14} color='#000' name='chevron-thin-down' /> */}
                        </View>
                    )
            ...
            <ScrollView style={styles.scrollStyle}>
                <View style={styles.commoditylist}>
                    {commodityData.map(item => {
                        return (
                            <View style={styles.commodityItem} key={item.index}>
                                <Image style={styles.commodityImage} source={item.image}></Image>
                                <Text style={styles.commodityText}>{item.title}</Text>
                                <View style={styles.priceView}>
                                    <View style={styles.priceStyle}>
                                        <Icon size={22} color='#fa3d3c' source={require('../../../assets/images/coins-icon.png')}></Icon>
                                        <Text style={styles.priceNum}>{item.num}</Text>
                                    </View>
                                    <Text style={styles.priceTime}>{item.time}</Text>
                                </View>
                                <View style={styles.publisherView}>
                                    <Avatar.Image style={styles.publisherAvatar} size={32} source={item.avatar} ></Avatar.Image>
